<x-guest-layout>
    ...
    <div class="flex max-h-screen w-full">
            ...
            <div class="bg-green-600 h-screen">
                <div class="p-7 flex justify-end h-[20vh]">
                    <div class="flex items-center justify-center">
                        ...
                        <a href="{{ url('/') }}"
                            class="relative inline-flex items-center justify-center p-4 px-10 py-3 overflow-hidden font-medium text-indigo-600 transition duration-300 ease-out border-2 border-white rounded-lg shadow-md group">
                            <span
                            ...
                            <span
                                class="absolute flex items-center justify-center w-full h-full text-white transition-all duration-300 transform group-hover:translate-y-full ease">Back
                                to website</span>
        ...
        <div class="w-1/2 flex flex-col justify-center mt-10">
            <div class="ml-48">
                <div class="flex w-2/3 justify-end mb-2">
                ...
                <form action="{{ route('login') }}" method="POST">
                    ...
                    <div class="flex items-start w-2/3">
                        <div>
                            ...
                            <label for="remember_me"
                                class="ms-2 me-36 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Remember Me') }}</label>
                        </div>
                        <div>
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}"
                                    class=" text-sm text-blue-700 hover:underline dark:text-blue-500">{{ __('Forgot Password?') }}</a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="">
                        <button type="submit"
                            class="w-2/3 text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 mt-8">{{ __('Sign In') }}</button>
                    </div>
                    ...
                    @if (Route::has('register'))
                        <div class="flex items-center justify-end mt-10 w-2/3">
                            <a href="{{ route('register') }}" class="ml-5 underline hover:text-blue-500">Create an
                                account</a>
                        </div>
                    @endif
